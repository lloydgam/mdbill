...
namespace OWINTest
{
    class Program
    {
        ...
        {
            string baseAddress = "http://localhost:52080";
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                Console.WriteLine(String.Format("Bearer {0}", token.AccessToken));
                httpClient.DefaultRequestHeaders.Add("Authorization", String.Format("Bearer {0}", token.AccessToken));
                HttpResponseMessage response = httpClient.GetAsync("api/information").Result;
